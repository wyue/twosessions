...
Magazine.prototype.generate=function(){
	...
	// 获取全部数据文件
	self.dataFiles=[self.dir+'/front_cover.json'];  //获取首页json数据
	for(var i=1;;i++){                              //for 循环获取每一个json数据的path
	...
	self.dataFiles.push(self.dir+'/back_cover.json');//获取底面的json数据 
	...
	async.eachSeries(self.dataFiles,function(filePath,callback){
		...
		// 复制img图片文件目录
		ncp('img',distDir+'/img',function(err){
			if (err) {
				return console.error(err);
			}else{
				console.log('img图片文件目录复制..OK');
			}
		});
		// 复制css文件目录
		ncp('css',distDir+'/css',function(err){
			if (err) {
				return console.error(err);
			}else{
				console.log('css文件目录复制..OK');
			}
		});
		// 复制js文件目录
		ncp('js',distDir+'/js',function(err){
			if (err) {
				return console.error(err);
			}else{
				console.log('js文件目录复制..OK');
			}
		});
	});
};
